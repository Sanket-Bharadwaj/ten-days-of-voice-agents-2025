...
async def complete_order(ctx: RunContext[Userdata]) -> str:
    ...
    try:
        ...
        return (
            f"🎉 PERFECT! Your {order.size} {order.drinkType} with {order.milk} milk"
            f"{extras_text} is confirmed, {order.name}! \n\n"
            "⏰ We're preparing your drink now - it'll be ready in a few minutes!"
        )
...
class BaristaAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            🏪 You are a FRIENDLY and PROFESSIONAL barista at our cafe.
