...
- Tools:
    ...
    - ingredients_for (natural language ingredient helper)
...
def list_orders_db(limit: int = 10, customer_name: Optional[str] = None) -> List[dict]:
    ...
    if customer_name:
        cur.execute(
            "SELECT * FROM orders WHERE LOWER(customer_name) = LOWER(?) ORDER BY created_at DESC LIMIT ?",
            (customer_name, limit),
        )
    else:
...
async def place_order(
    ...
    try:
        ...
        logger.warning(f"Could not schedule delivery simulation for {order_id}: {e}")
...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Jarvis', a helpful assistant for a local Indian grocery store.
            ...
            4. High-level ingredients: Understand natural language like 'ingredients for maggi for two people'.
            5. Orders: Place orders with express tracking.
            6. Cancellation: You can CANCEL an order if the user asks, provided it's not delivered yet.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🇮🇳" * 12)
    logger.info("🚀 STARTING FOOD AGENT (Indian Context + Auto-Tracking)")
