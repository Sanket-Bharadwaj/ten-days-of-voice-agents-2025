...
class FraudCase:
    ...
    status: str = "pending"  # pending | confirmed_fraud | confirmed_safe
...
class FraudDatabase:
    ...
    def get_fraud_case_by_name(self, name: str) -> Optional[FraudCase]:
        """Get fraud case by customer first name"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
            ...
            print(f"❌ Error getting fraud case by name: {e}")
            return None
